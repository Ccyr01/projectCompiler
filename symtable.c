/* Christian Cyr 
 last edit 4/8/2022
 Insert and search symbol table methods. Insert uses Search method before adding symbol
 Compareformals function returns 1 if true and 0 for false. Display shows what we
 have in the symbol table, along with level and offset.
*/
...
//pre:p->fparms(which is pointer to parameters of the function in the AST)&
//& $3 from Yacc (which is args)
//post:1 or 0 
int CompareFormals(struct ASTnode *formals,struct ASTnode *actual){
  if((formals==NULL)&&(actual == NULL))return 1;
  if((formals==NULL)||(actual == NULL))return 0;
  if((formals->declared_type)!=(actual->declared_type))return 0;
    return CompareFormals(formals->next, actual->next);
